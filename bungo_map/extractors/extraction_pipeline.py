...
class ExtractionPipeline:
    ...
    def extract_places(self, work_id: int, content: str) -> List[Dict[str, Any]]:
        """作品から地名を抽出"""
        # 1. センテンス分割
        sentences = self.sentence_extractor.split_into_sentences(content)
        
        # 2. 各センテンスから地名抽出
        all_places = []
        for sentence in sentences:
            result = self.process_sentence(sentence, work_id)
            if result['places']:
