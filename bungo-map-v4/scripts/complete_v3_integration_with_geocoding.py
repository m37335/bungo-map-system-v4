...
# -*- coding: utf-8 -*-
"""
🚀 v3→v4完全統合パイプライン + AI Geocoding

v3の優秀な以下機能をv4データベースに完全統合:
- AozoraContentProcessor (青空文庫処理)
- 高度な地名抽出システム (Simple + Enhanced)
- AI文脈判断型Geocoding
- 適切なセンテンス処理とコンテキスト保存
"""
...
class V3ToV4IntegratorWithGeocoding:
    """v3→v4完全統合クラス（AI Geocoding付き）"""
    ...
    def integrate_all_works(self):
        ...
        for i, work in enumerate(works, 1):
            ...
            try:
                ...
                print(f"🌍 AI Geocoding: {geocoded}件成功")
        ...
        # 最終結果
        print(f"\n{'='*80}")
        print(f"🎉 v3→v4完全統合 + AI Geocoding完了")
        print(f"📝 総センテンス数: {total_sentences}")
        print(f"🗺️ 総地名数: {total_places}")
        print(f"🌍 AI Geocoding成功: {total_geocoded}")
        if total_places > 0:
    ...
    def _process_aozora_content(self, work: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            content_result = self.aozora_processor.get_work_content_with_context(aozora_url)
            
            if content_result and content_result.sentences:
                print(f"✅ コンテンツ取得成功 (センテンス数: {len(content_result.sentences)})")
                return {
    ...
    def _process_sentences(self, work_id: int, content_data: Dict[str, Any]) -> List[Dict[str, Any]]:
        """v3のsentence contextを使用してセンテンス処理"""
        sentence_contexts = content_data['sentences']
    ...
    def _execute_ai_geocoding(self, places: List[Dict[str, Any]]) -> int:
        """v3のAI文脈判断型Geocodingでジオコーディング実行"""
        if not places:
            return 0
        ...
        print(f"🌍 AI Geocoding実行: {len(unique_places)}の固有地名")
...
def main():
    """メイン実行関数"""
    print("🚀 v3→v4完全統合パイプライン + AI Geocoding 開始")
    print("=" * 80)
    
    integrator = V3ToV4IntegratorWithGeocoding()
    ...
    print(f"\n{'='*80}")
    print(f"📊 v3→v4完全統合 + AI Geocoding結果")
    print(f"{'='*80}")
    ...
    print(f"✅ v3の地名抽出・AI Geocoding機能統合完了！")
